...
This function uses the supabase HTTP client to talk to Supabase REST API.
It performs short-lived HTTP requests (no DB persistent connections) so it's safe
...
Note: Vercel's Python runtime will call this file as a function. We implement
the expected handler signature using the `vercel` adapter style: function `handler(req, res)`.
"""
import os
import json
